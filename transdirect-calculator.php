...
<style>
	...
	#trans_frm {right: 0;width:305px; text-indent:0; padding:5px; margin-bottom:20px; border:1px solid #ededed;margin-top: 0px;background-color:#FFFFFF;position:relative;}
	#trans_frm h4 {margin:0 0 0 0 !important;}
	#shipping_type {border-top:1px solid #ededed; padding-top:10px; margin-top:10px; text-align:left;}
	p.form-row-wide, p.form-row-small {margin:5px 0 !important;}
	p.form-row-wide input[type="text"] {width:202px;}
	p.form-row-small input[type="text"] {width:135px; text-align:right}
	...
	span.close-option {top: -15px;right: 5px;position: relative;}
</style>
...
<?php if (WC()->session->chosen_shipping_methods[0] == 'woocommerce_transdirect' && !isset($_SESSION['price'])) {
	...
	<div class="shipping_calculator" id="trans_frm">
		...
		<section class="shipping-calculator-form1">
			...
			<?php } ?>
			<p>
				<button type="button" name="calc_shipping" value="1" class="button" onclick="javascript:validate('<?php echo plugins_url('quotes.php', __FILE__); ?>');">
